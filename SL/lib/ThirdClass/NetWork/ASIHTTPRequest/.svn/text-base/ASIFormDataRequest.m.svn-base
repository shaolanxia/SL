...
//	//NSString *String = [NSMakeCollectable(CFURLCreateStringByAddingPercentEscapes(kCFAllocatorDefault, (CFStringRef)string, NULL, CFSTR(":/?#[]@!$ &'()*+,;=\"<>%{}|\\^~`"), CFStringConvertNSStringEncodingToEncoding([self stringEncoding]))) autorelease];
//	
//	//add by liuwei
//	NSString *String = [NSMakeCollectable(CFURLCreateStringByAddingPercentEscapes(kCFAllocatorDefault, (CFStringRef)string, NULL, CFSTR(":/?#[]@!$ &'()*+,;=\"<>%{}|\\^~`"), CFStringConvertNSStringEncodingToEncoding([self stringEncoding]))) autorelease];
//    
//    //*** BEGIN *** 2012-03-22 xiongguofeng ADD
//    //如果传入内容中有字符‘~’，上面的转换只会转换为"%7E"，这并不是正确的utf-8编码，增加一行直接替换
//    String = [String stringByReplacingOccurrencesOfString:@"%7E" withString:@"%EF%BD%9E"];
//    //*** END ***
//    
